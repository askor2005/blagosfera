...
function getSimpleResultVoting(voting, isSecretVoting, isValid, isCandidate, successDecree, failDecree) {
		...
		if (votingItem.value == "ABSTAIN") {
			...
			if (isCandidate){
				...
				if (votingItem.votesPercent >= percentForWin) {
					winners.push({name : candidate.fullName, votesPercent : votingItem.votesPercent});
				}
			} else {
				votingItemsResult += "За вариант \"" + votingItem.value + "\"";
				if (votingItem.votesPercent >= percentForWin) {
					winners.push({name : votingItem.value, votesPercent : votingItem.votesPercent});
				}
			}
	...
	if (abstainVotingItem != null) {
		votingItemsResult += "Воздержались от голосования - " + abstainUserIds.length + " " + stringForms(abstainUserIds.length, "человек", "человека", "человек") + " (" + getPercent(abstainVotingItem.votesPercent) + "%)<br/>";
	...
	if (isValid) {
		...
		var winnerText4 = "\"";
		if (isCandidate) {
			winnerText1 = "<br/><b>Победили следующие кандидаты:</b><br/>";
		...
		if (winners.length > 1) {
			...
			for (var index in winners) {
				...
				votingItemsResult += i + ") " + winnerText3 + winner.name + winnerText4 + " - " + (winner.votesPercent == 100 ? "решение принято единогласно" : (winner.votesPercent > 50 ? "решение принято большинством" : "решение принято на основе заданных параметров голосования")) +  "<br/>";
				winnersNameArr.push(winner.name);
			}
		} else if (winners.length == 1) {
			var winner = winners[0];
			votingItemsResult += winnerText2 + winnerText3 + winner.name + winnerText4 + " - " + (winner.votesPercent == 100 ? "решение принято единогласно" : (winner.votesPercent > 50 ? "решение принято большинством" : "решение принято на основе заданных параметров голосования")) +  "<br/>";
			winnersNameArr.push(winner.name);
		}
...
function getProContraResultVoting(voting, isSecretVoting, isValid, successDecree, failDecree) {
	...
	for (var index in voting.votingItems) {
		...
		switch (votingItem.value) {
			case "PRO": {
				...
				isMoreVictory = votingItem.votesPercent > 50;
	...
	if (abstainVotingItem != null) {
	    result += "Воздержались от голосования - " + abstainUserIds.length + " " + stringForms(abstainUserIds.length, "человек", "человека", "человек") + " (" + getPercent(abstainVotingItem.votesPercent) + "%)<br/>";
	    if (!isSecretVoting && abstainUserIds.length > 0) {
	...
	if (isValid) {
		...
		} else if (isMoreVictory) {
			result += "<br/><b>Победил вариант \"ЗА\"</b>. Решение принято большинством";
		} else {
			result += "<br/><b>Победил вариант \"ЗА\"</b>. Решение принято на основе заданных параметров голосования";
		}
		if (successDecree != null) {
	...
	if (isSecretVoting) {
		if (newVotingItems != null && newVotingItems.length > 0) {
			...
			for (var index in newVotingItems) {
				...
				variantsArr.push(votingItem.value);
			}
			result += variantsArr.join("; ");

			if (abstainUserIds.length > 0) {
				result += "<br/>Воздержались от выдвижения вариантов - " + abstainUserIds.length + " " +
						stringForms(abstainUserIds.length, "человек", "человека", "человек") + " (" + getPercent(abstainVotingItem.votesPercent) + "%)";

			}


			if (successDecree != null) {
				result += "<br/><br/><b>Постановили:</b><br/>" + successDecree + " " + variantsArr.join("; ") + ".";
			}
		} else {
		...
		if (newVotingItems != null && newVotingItems.length > 0) {
			...
			for (var voterId in userItems) {
				...
				for (var index in userItem) {
					...
					variantsArr.push(item.value);
					userVariants.push(item.value);
				}
				if (userVariants.length > 0) {
					userVariantsAll.push("Участник " + voter.fullName + (voter.sex ? " предложил " : " предложила ") + "варианты:<br/>" + userVariants.join("; "));
				}
			}
			result += userVariantsAll.join("<br/>");

			if (abstainUserIds.length > 0) {
				result += "<br/>Воздержались от выдвижения вариантов - " + abstainUserIds.length + " " +
						stringForms(abstainUserIds.length, "человек", "человека", "человек") + " (" + getPercent(abstainVotingItem.votesPercent) + "%)<br/>" +
						"(" + getUsersFioByComma(abstainUserIds) + ")";
			}
