...
begin
  ...
  select id into regionFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_REGION' and fields_group_id = groupId;
  select id into areaFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_DISTRICT' and fields_group_id = groupId;
  select id into cityFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_CITY' and fields_group_id = groupId;
  select id into streetFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_STREET' and fields_group_id = groupId;
  select id into houseFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_BUILDING' and fields_group_id = groupId;
  select id into roomFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_ROOM' and fields_group_id = groupId;

  select id into regionDescriptionFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_REGION_DESCRIPTION' and fields_group_id = groupId;
  select id into areaDescriptionFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_DISTRICT_DESCRIPTION' and fields_group_id = groupId;
  select id into cityDescriptionFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_CITY_DESCRIPTION' and fields_group_id = groupId;
  select id into streetDescriptionFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_STREET_DESCRIPTION' and fields_group_id = groupId;
  select id into houseDescriptionFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_BUILDING_DESCRIPTION' and fields_group_id = groupId;
  select id into roomDescriptionFieldId from fields where internal_name = 'REGISTRATOR_OFFICE_ROOM_DESCRIPTION' and fields_group_id = groupId;
  ...
  FOR sharer IN select * from sharers where id in (select sharer_id from community_members where id in (select member_id from community_members_posts where post_id in (select id from community_posts where mnemo like '%registrator%')))
  LOOP
    RAISE NOTICE 'sharer: %', sharer;

    -- РЕГИОН * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *

    select string_value into regionFieldValue from field_values where object_type='SHARER' and object_id=sharer.id and field_id = regionFieldId;
    ...
    IF flag = TRUE
    THEN
      UPDATE field_values SET string_value=region WHERE object_type='SHARER' and object_id=sharer.id and field_id=regionFieldId;

      UPDATE field_values SET string_value=regionDescription WHERE object_type='SHARER' and object_id=sharer.id and field_id=regionDescriptionFieldId;

      insert into field_values (id, hidden, object_type, object_id, field_id, string_value, file_url)
      select nextval('seq_field_values'), true, 'SHARER', sharer.id, regionDescriptionFieldId, regionDescription, null
      WHERE NOT EXISTS (SELECT 1 FROM field_values WHERE object_type='SHARER' and object_id=sharer.id and field_id=regionDescriptionFieldId);
    END if;
    ...
    select string_value into areaFieldValue from field_values where object_type='SHARER' and object_id=sharer.id and field_id = areaFieldId;
    ...
    IF flag = TRUE
    THEN
      UPDATE field_values SET string_value=area WHERE object_type='SHARER' and object_id=sharer.id and field_id=areaFieldId;
    END if;

    UPDATE field_values SET string_value=areaDescription WHERE object_type='SHARER' and object_id=sharer.id and field_id=areaDescriptionFieldId;

    insert into field_values (id, hidden, object_type, object_id, field_id, string_value, file_url)
    select nextval('seq_field_values'), true, 'SHARER', sharer.id, areaDescriptionFieldId, areaDescription, null
    WHERE NOT EXISTS (SELECT 1 FROM field_values WHERE object_type='SHARER' and object_id=sharer.id and field_id=areaDescriptionFieldId);

    RAISE NOTICE 'area: %', '[' || areaFieldValue || '] -> ' || '[' || area || ']  [' || areaDescription || ']';

    -- НАСЕЛЁННЫЙ ПУНКТ * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *

    select string_value into cityFieldValue from field_values where object_type='SHARER' and object_id=sharer.id and field_id = cityFieldId;
    ...
    IF flag = TRUE
    THEN
      UPDATE field_values SET string_value=city WHERE object_type='SHARER' and object_id=sharer.id and field_id=cityFieldId;
    END if;

    UPDATE field_values SET string_value=cityDescription WHERE object_type='SHARER' and object_id=sharer.id and field_id=cityDescriptionFieldId;

    insert into field_values (id, hidden, object_type, object_id, field_id, string_value, file_url)
    select nextval('seq_field_values'), true, 'SHARER', sharer.id, cityDescriptionFieldId, cityDescription, null
    WHERE NOT EXISTS (SELECT 1 FROM field_values WHERE object_type='SHARER' and object_id=sharer.id and field_id=cityDescriptionFieldId);

    RAISE NOTICE 'city: %', '[' || cityFieldValue || '] -> ' || '[' || city || ']  [' || cityDescription || ']';

    -- УЛИЦА * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *

    select string_value into streetFieldValue from field_values where object_type='SHARER' and object_id=sharer.id and field_id = streetFieldId;
    ...
    IF flag = TRUE
    THEN
      UPDATE field_values SET string_value=street WHERE object_type='SHARER' and object_id=sharer.id and field_id=streetFieldId;
    END if;

    UPDATE field_values SET string_value=streetDescription WHERE object_type='SHARER' and object_id=sharer.id and field_id=streetDescriptionFieldId;

    insert into field_values (id, hidden, object_type, object_id, field_id, string_value, file_url)
    select nextval('seq_field_values'), true, 'SHARER', sharer.id, streetDescriptionFieldId, streetDescription, null
    WHERE NOT EXISTS (SELECT 1 FROM field_values WHERE object_type='SHARER' and object_id=sharer.id and field_id=streetDescriptionFieldId);

    RAISE NOTICE 'street: %', '[' || streetFieldValue || '] -> ' || '[' || street || ']  [' || streetDescription || ']';

    -- Дом * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *

    select string_value into houseFieldValue from field_values where object_type='SHARER' and object_id=sharer.id and field_id = houseFieldId;
    ...
    IF flag = TRUE
    THEN
      UPDATE field_values SET string_value=house WHERE object_type='SHARER' and object_id=sharer.id and field_id=houseFieldId;
    END if;

    UPDATE field_values SET string_value=houseDescription WHERE object_type='SHARER' and object_id=sharer.id and field_id=houseDescriptionFieldId;

    insert into field_values (id, hidden, object_type, object_id, field_id, string_value, file_url)
    select nextval('seq_field_values'), true, 'SHARER', sharer.id, houseDescriptionFieldId, houseDescription, null
    WHERE NOT EXISTS (SELECT 1 FROM field_values WHERE object_type='SHARER' and object_id=sharer.id and field_id=houseDescriptionFieldId);

    RAISE NOTICE 'house: %', '[' || houseFieldValue || '] -> ' || '[' || house || ']  [' || houseDescription || ']';

    -- Квартира * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *

    select string_value into roomFieldValue from field_values where object_type='SHARER' and object_id=sharer.id and field_id = roomFieldId;
    ...
    UPDATE field_values SET string_value=roomDescription WHERE object_type='SHARER' and object_id=sharer.id and field_id=roomDescriptionFieldId;

    insert into field_values (id, hidden, object_type, object_id, field_id, string_value, file_url)
    select nextval('seq_field_values'), true, 'SHARER', sharer.id, roomDescriptionFieldId, roomDescription, null
    WHERE NOT EXISTS (SELECT 1 FROM field_values WHERE object_type='SHARER' and object_id=sharer.id and field_id=roomDescriptionFieldId);
