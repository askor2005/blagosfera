...
import ru.radom.blagosferabp.activiti.custom.component.storage.StorageGetTask;
import ru.radom.blagosferabp.activiti.custom.component.storage.StorageStoreTask;
import ru.radom.blagosferabp.activiti.custom.component.storage.behaviours.StorageGetTaskBehaviour;
 ...
 */
@Component
public class StorageGetTaskModelBundle implements ModelBundle {

    public final static String STORAGE_GET_TASK = "storageGetTask";

    public final static String VALUES_MAPPING_PROPERTY = "values_mapping";

    public final static String LAST_MODIFIED_DATE_PROPERTY = "last_modified";
    ...
    private void postConstruct() {
        storageGetTaskStencil = Stencil.builder()
            ...
            .property(
                Property.builder()
                    .id(LAST_MODIFIED_DATE_PROPERTY)
                    .title("Последняя модификация")
                    .description("Будут взяты значения не моложе даты, формат dd.MM.yyyy и time")
    ...
    public String getStencilId() {
        return STORAGE_GET_TASK;
    }

    @Override
    public Stencil getStencil() {
        return storageGetTaskStencil;
    }
    ...
    public BpmnParseHandler getBpmnParseHandler() {
        return new AbstractActivityBpmnParseHandler<StorageGetTask>() {
            @Override
            protected Class<? extends BaseElement> getHandledType() {
                return StorageGetTask.class;
            }

            @Override
            protected void executeParse(BpmnParse bpmnParse, StorageGetTask element) {
                ActivityImpl activity = createActivityOnCurrentScope(bpmnParse, element, BpmnXMLConstants.ELEMENT_TASK_SERVICE);
                ExpressionManager expressionManager = bpmnParse.getExpressionManager();
                Map<Expression, String> dataMapping = null;
                if(element.getDataMapping() != null) {
                    dataMapping = new HashMap<>();
                    for (Map.Entry<String, String> entry : element.getDataMapping().entrySet()) {
                        Expression key = expressionManager.createExpression(entry.getKey());
                        dataMapping.put(key, entry.getValue());
                    }
                }
                String lastModifiedDate = element.getLastModifiedDate();
                StorageGetTaskBehaviour behaviour = applicationContext.getBean(
                    StorageGetTaskBehaviour.class,
                    dataMapping,
                    lastModifiedDate == null ? null : expressionManager.createExpression(lastModifiedDate),
