 ...
 * Конвертер BPMN процесса в формат JSON
 */
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {

    protected static final Logger log = LoggerFactory.getLogger(BpmnJsonConverter.class);

    protected final ObjectMapper objectMapper = new ObjectMapper();

    protected final Map<Class<? extends BaseElement>, BaseBpmnJsonConverter> convertersToJsonMap =
            new HashMap<>();
    protected final Map<String, BaseBpmnJsonConverter> convertersToBpmnMap =
            new HashMap<>();

    //Заполняем дефолтные типы
    {
        ...
        try {
            for (Map.Entry<String, Class<? extends BaseBpmnJsonConverter>> entry : convertersToBpmnMap.entrySet()) {
                this.convertersToBpmnMap.put(entry.getKey(), entry.getValue().newInstance());
            }
            for (Map.Entry<Class<? extends BaseElement>, Class<? extends BaseBpmnJsonConverter>> entry : convertersToJsonMap.entrySet()) {
                this.convertersToJsonMap.put(entry.getKey(), entry.getValue().newInstance());
            }
    ...
    protected final List<String> DI_CIRCLES = new ArrayList<String>();
    protected final List<String> DI_RECTANGLES = new ArrayList<String>();
    protected final List<String> DI_GATEWAY = new ArrayList<String>();
    ...
    /**
     * Добавляет расширенный ковертер
     */
    public void addConverter(ExtendedBaseBpmnJsonConverter jsonConverter) {
        jsonConverter.fillTypes(convertersToBpmnMap, convertersToJsonMap);
        jsonConverter.fillCirclesTypes(DI_CIRCLES);
        jsonConverter.fillRectanglesTypes(DI_RECTANGLES);
        jsonConverter.fillGatewayTypes(DI_GATEWAY);
    }
    ...
    protected void processFlowElement(FlowElement flowElement, FlowElementsContainer container, BpmnModel model,
        ...
        if (converter != null) {
            try {
                converter.convertToJson(flowElement, this, model, container, shapesArrayNode, containerX, containerY);
            } catch (Exception e) {
    ...
    protected void processArtifacts(FlowElementsContainer container, BpmnModel model, ArrayNode shapesArrayNode,
        ...
        for (Artifact artifact : container.getArtifacts()) {
            BaseBpmnJsonConverter converter = convertersToJsonMap.get(artifact.getClass());
            if (converter != null) {
                try {
                    converter.convertToJson(artifact, this, model, container,
    ...
    public void processJsonElements(JsonNode shapesArrayNode, JsonNode modelNode, BaseElement parentElement, Map<String, JsonNode> shapeMap,
        ...
        for (JsonNode shapeNode : shapesArrayNode) {
            String stencilId = BpmnJsonConverterUtil.getStencilId(shapeNode);
            BaseBpmnJsonConverter converter = convertersToBpmnMap.get(stencilId);
            try {
                converter.convertToBpmnModel(shapeNode, modelNode, this, parentElement, shapeMap, bpmnModel);
            } catch (Exception e) {
