...
<%@ include file="communityBatchVotingsGrid.jsp"  %>
<%@ include file="communityBatchVotingVotersGrid.jsp"  %>
<script>
    ...
    function loadBatchVotingsPageData(communityId, callBack) {
        $.radomJsonPost(
                "/group/" + communityId + "/batch_votings_page_data.json",
                {},
                callBack
        );
    }

    function initControlsView(votingsAdmin) {
    ...
    function initControls(votingsAdmin) {
        ...
        $("#batchVotingVoters").on("shown.bs.modal", function () {
            initBatchVotingVotersGrid(communityId, currentBatchVotingId);
        });
        ...
        if (votingsAdmin) {
            $("#showMyBatchVotings").click(function(){
                filterParameters.showMyBatchVotings = $(this).is(":checked");
                filterBatchVotings();
            });
        }
    ...
    $(document).ready(function() {
        ...
        initBatchVotingsGrid(communityId, filterParameters);
        loadBatchVotingsPageData(communityId, function(response){
            initControlsView(response.votingsAdmin);
            initControls(response.votingsAdmin);
        });
    });
</script>

<t:insertAttribute name="communityHeader" />
<h2>Собрания сообщества с моим участием</h2>
<hr/>
<div>
    ...
    <div id="filterBlock" style="display: none;">
        ...
        <div class="form-group" id="showMyBatchVotingsBlock" style="display: none;">
            <label>
                Показать только собрания с моим участием
                <input type="checkbox" id="showMyBatchVotings" checked="checked"/>
            </label>
        </div>
