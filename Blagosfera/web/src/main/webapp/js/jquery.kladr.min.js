	...
	$.kladr.validate = function (query) {
		...
		switch (query.type) {
			...
			case type.city:
				...
				break;
			case type.street:
				if (query.parentType == type.region) {
					query.parentType = type.city;
					query.cityId = type.regionId;
				}
...
(function ($, window, document, undefined) {
	...
	$.kladr = $.extend($.kladr, {
		 ...
		 * @returns {string}
		 */
		getAddress: function (selector, build, $zipInput) {
			...
			$inputs.each(function () {
				...
				if ($this.attr('data-kladr-id')) {
					...
					if ($this.attr('data-kladr-type') == "building") {
						if ($this.val() != null && $this.val() != "") {
							filtered[$this.attr('data-kladr-type')] = {contentType : "building", name: $this.val(), type: "Дом", typeShort: "д"}
						}
					} else {
			...
			return (build || $.kladr.buildAddress)(sorted, $zipInput);
		},
		 ...
		 * @returns {string}
		 */
		buildAddress: function (objs, $zipInput) {
			var lastIds = [],
				address = '',
				zip = '';
			...
			$.each(objs, function (i, obj) {
				...
				if ($.type(obj) === 'object') {
					...
					if (obj.contentType == "city" || obj.type == "Город" || obj.contentType == "building") {
			...
			// Если поле для индекса и оно пустое или значение можно менять
			if (zip && $zipInput != null && $zipInput.length > 0 && ($zipInput.val() == null || $zipInput.val() == '' || $zipInput.attr("can_be_changed") == "true")) {
				$zipInput.val(zip);
				$zipInput.blur();
				$zipInput.attr("can_be_changed", "true");
			}
			if ($zipInput != null && $zipInput.length > 0 && $zipInput.val() != null) {
				zip = $zipInput.val();
			}
